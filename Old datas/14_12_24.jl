...
let    #@
    ...
    nt = Int(log(2, dh))
    nb = 2 #number of bath  
    ek = 140 #length of the bath chain  
    #------------------------------------------------------------------------------------------------------   
    # ITensor space 
    # Dot position
    sysp = Int(nt + 1)
    ...
    #-------------------------------------------------------------------------------------------------------
    # gates
    chain_ham = unit_gates_fm_in(recurr_list, ek, dh, tau, nb, stbath, η)
    sys_ham = Hs_chain_gen_fm_in(stbath, ϵ, ω0, λ, dh, ek, nb)
    #---------------------------------------------------------------------------
    # observable
    pop2 = OpSum()
    ...
    println("Time = ", 0)
    println("Phonon = ", pd)
    write_for_loop(file_name_txt, string(1), "cutoff = $cutoff1, maxdim = $maxdim1, dt = $tau, beta = $temps", string(pop_ph))
    ...
    for t = 1:nst
        ...
        if t % 10 == 0
            ...
            ρ = dynamical_trunc_MP_in(ρ, maxdim1, maxdim2, steepness, max_frac, sysp, p_gap)
        end
        orthogonalize!(ρ, Int(floor((bpose_st + bpose_en) / 2)))
        #-------------------------------------------------------------------------------------------------------
        # Measurement
        pd = exp(loginner(pop2_op, ρ; cutoff=0.1 * cutoff1))
        append!(pop_ph, pd)
        println("Time = ", t * tau)
        println("Phonon = ", pd)
        println("beta = ", temps)
        ...
        if t % 320 == 0
            ...
            p1 = plot()
            title_string = L"\Gamma _{0} = %$(Γ0_list[1]), \beta = %$temps, m_{dim} = %$maxdim1, trunc = [%$(p_gap), \ %$(max_frac)]"
    ...
    write_for_loop(file_name_txt, string(2), "cutoff = $cutoff1, beta = $temps, maxdim = $maxdim1, dt = $tau", string(real(pop_ph)))

    #-------------------------------------------------------------------------------------------------------
    # Final plotting and phonon density matrix 
    p1 = plot()
    title_string = L"\Gamma _{0} = %$(Γ0_list[1]), \beta = %$temps, m_{dim} = %$maxdim1, trunc = %$(p_gap)"
